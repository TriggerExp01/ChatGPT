...
本仓库已从旧项目清理为 Unity + TEngine 工程骨架，当前 Dev 分支已经接入 `Cultivation / 修仙养成` 的入口与部分业务原型，并正式进入《仙途·天命》完整游戏持续开发。

当前默认协作基线是：**TEngine 骨架 + 《仙途·天命》东方玄幻卡牌构筑 Roguelite**。正式终态、v1.0 范围、实施顺序与完成判定以 `Doc/完整游戏完成合同.md` 为准。用户已授权持续完成整个游戏，阶段内的产品、玩法、美术和技术选择由执行者依据合同、GDD 和当前证据自行收束，不再逐项询问。
...
- 当前 Dev 分支已接入 Cultivation 原型入口；它是完整游戏的现有基础，不是最终完成状态。
- 后续新增阶段必须先从完整游戏合同和 GDD 推导该阶段的目标、玩法边界、UI 风格和验收标准，并直接推进。
- Editor 快速入口只视为原型调试入口；正式验收应优先走 Procedure -> GameApp -> Cultivation 业务入口，并区分两条链路的验证结果。
- 不允许把旧肉鸽项目的玩法、UI 风格或资源作为默认方向继续继承。

## 默认协作规则

- 完整游戏目标范围内的产品方向、玩法设计、UI 风格、资源选择和架构边界按合同与现有参考自行决策并执行；不得用等待确认代替推进。
