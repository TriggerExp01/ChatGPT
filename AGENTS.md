...
4. `Doc/框架使用指南.md`
5. `Doc/开发日志.md`
6. `Doc/四Agent阶段开发工作流.md`

涉及 Unity MCP、Play Mode、Editor 状态读取、控制台检查时，再读取：

7. `Doc/Unity MCP 在 Codex 中连接固化说明.md`
...
- 除非用户已经明确授权连续推进某个阶段，否则重要方案必须先给用户确认，再执行代码修改、资源导入、Unity 验证或提交。
- 重要决定包括但不限于：产品定位、UI 风格、资源来源、阶段边界、架构方向、配置表结构、资源管线、验收口径和商业化展示目标。
- 经用户确认的重要决定必须固化到现有有效文档中，优先写入 `Doc/类吸血鬼幸存者阶段开发路线.md`、`Doc/肉鸽系统设计总览.md`、`Doc/开发日志.md` 或本文件，避免只停留在聊天记录中。
- 讨论阶段可以给方案、风格稿和取舍建议；未经确认，不把讨论方案直接落成代码或项目资源。
...
当用户给出游戏成品描述、要求自动拆分阶段、要求多个 Agent 接力，或要求“开发 - Unity MCP 验收 - 打回修复 - 体验复盘”闭环时，必须启用 `Doc/四Agent阶段开发工作流.md`。

四个角色固定为：

- 总策划 / 制作人 Agent：负责产品目标提炼、阶段开发合同、玩法策划、数值目标和验收标准，不直接改代码。
- 主开发 Agent：负责当前阶段代码、资源、配置、测试和文档更新，输出阶段交付包。
- Unity MCP 验收 Agent：负责真实 Unity MCP Play Mode 验收，有一票否决权，不开发新功能。
- 体验复盘 / 调优 Agent：负责玩家体验、节奏、构筑爽感、UI 可读性和展示图价值复盘，不推翻 Unity MCP 验收结论。

四 Agent 流程的硬规则：

- 主开发 Agent 未拿到“验收通过”前，不进入下一阶段。
- Unity MCP 验收 Agent 输出“验收不通过”或“验收阻塞”时，主开发 Agent 只能修复当前阶段。
- 体验复盘建议必须拆成当前阶段返工建议和下一阶段建议，不能把未来阶段直接并入当前实现。
- 验收通过后，主开发 Agent 必须更新文档并执行阶段 commit。
